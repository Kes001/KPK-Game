...
void ControlBall (Ball* ball1, Ball* ball2)
    ...
    if (txGetAsyncKeyState (VK_UP))    (*ball2).vy = (*ball2).vy - 1;
    if (txGetAsyncKeyState (VK_DOWN))  (*ball2).vy = (*ball2).vy + 1;

    if (txGetAsyncKeyState ('D')) (*ball1).vx = (*ball1).vx + 1;
    if (txGetAsyncKeyState ('A')) (*ball1).vx = (*ball1).vx - 1;
    if (txGetAsyncKeyState ('W')) (*ball1).vy = (*ball1).vy - 1;
    if (txGetAsyncKeyState ('S')) (*ball1).vy = (*ball1).vy + 1;

    if (txGetAsyncKeyState (VK_SPACE)) (*ball1).vx = (*ball1).vy =  0;
    if (txGetAsyncKeyState (VK_SPACE)) (*ball2).vx = (*ball2).vy = 50;
    }
